...
class HistoryStoneBar extends Component {
  initTimeline = () => {
    ...
    function setTimelineWidth(timelineComponents, width) {
      var timeSpan = daydiff(
        ...
        temptTimeSpanNorm = timeSpan / timelineComponents["eventsMinLapse"],
        timeSpanNorm = Math.round(temptTimeSpanNorm) + 4,
  ...
  */
  ...
  componentDidMount() {
    this.props.actions.handleGenerateYear();
  }

  componentDidUpdate(prevProps) {
    const { timelineStones, isGenerateYear } = this.props;

    if (prevProps.timelineStones !== timelineStones && isGenerateYear) {
      this.initTimeline();
      this.props.actions.handleClear("generateYear");
      this.props.actions.getPostsByYear({
        time: timelineStones[0].value,
        page: 0
      });
    }
  }
  ...
  _handleGetPost = item => {
    ...
    this.props.actions.handlePageChange(0);
    this.props.actions.handleCurYear({type: "normal", data: item });
  };

  render() {
    const { timelineStones, curYear, transform } = this.props;
    return (
      <React.Fragment>
        <div
          id="cd-horizontal-bar"
          className={classnames(
            { "vs-hidden": _.isEmpty(timelineStones) },
            ...
            <div className="timeline">
              <div className="events-wrapper">
                <div className="events">
                  <ol>
                    {timelineStones.map(item => {
                      return (
                        <li key={item.value}>
                          <a
                            href="#0"
                            data-date={`01/01/${item.value}`}
                            className={
                              item.value === curYear.value ? "selected" : ""
                            }
                            onClick={() => this._handleGetPost(item)}
                          >
                            {item.label}
                          </a>
                        </li>
...
export default HistoryStoneBar;
